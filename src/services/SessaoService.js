...
class SessaoService {
  async createSession(id_filme, id_sala, data_sessao) {
    try {
      // Chamando o repositório para criar a sessão passando o corpo com os dados
      const session = await SessaoRepository.createSession(
        id_filme,
        id_sala,
        data_sessao
      );
  ...
  async getSessionMonth() {
    try {
      ...
      const exibicao = filmsWithSessions
        .filter(
          (film) => film.data_lancamento <= now && film.sessoes.length > 0
        )
        .slice(0, 4); // Limitando a 4 filmes

      // Filmes em breve (com lançamento futuro)
      const emBreve = filmsWithSessions
        .filter((film) => film.data_lancamento > now)
        .slice(0, 4); // Limitando a 4 filmes
    ...
    try {
      ...
      if (response.success) {
        ...
        return {
          ...
          duracao: `${Math.floor(session.duracao / 60)}h ${
            session.duracao % 60
          }min`,
      ...
      if (response.success) {
        ...
        const mappedSession = {
          ...
          filme: {
            ...
            duracao: `${Math.floor(session.filme.duracao / 60)}h ${
              session.filme.duracao % 60
            }min`,
